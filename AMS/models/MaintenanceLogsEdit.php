 ...
 * Page class
 */
class MaintenanceLogsEdit extends MaintenanceLogs
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MaintenanceLogsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "MaintenanceLogsEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("log_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->log_id->setQueryStringValue($keyValue);
                $this->log_id->setOldValue($this->log_id->QueryStringValue);
            } elseif (Post("log_id") !== null) {
                $this->log_id->setFormValue(Post("log_id"));
                $this->log_id->setOldValue($this->log_id->FormValue);
            } else {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // schedule_id
        $this->schedule_id->setDbValueDef($rsnew, $this->schedule_id->CurrentValue, $this->schedule_id->ReadOnly);
        ...
        // performed_by
        $this->performed_by->setDbValueDef($rsnew, $this->performed_by->CurrentValue, $this->performed_by->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        if (isset($row['schedule_id'])) { // schedule_id
            $this->schedule_id->CurrentValue = $row['schedule_id'];
        }
        ...
        if (isset($row['performed_by'])) { // performed_by
            $this->performed_by->CurrentValue = $row['performed_by'];
        }
