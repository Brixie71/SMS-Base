...
/**
 * Table class for equipment_types
 */
class EquipmentTypes extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "equipment_types";
        ...
        $this->type_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->maintenance_interval = new DbField(
            $this, // Table
            'x_maintenance_interval', // Variable name
            'maintenance_interval', // Name
            '"maintenance_interval"', // Expression
            'CAST("maintenance_interval" AS varchar(255))', // Basic search expression
            3, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"maintenance_interval"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->maintenance_interval->InputTextType = "text";
        $this->maintenance_interval->Raw = true;
        $this->maintenance_interval->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->maintenance_interval->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['maintenance_interval'] = &$this->maintenance_interval;

        // requires_certification
        $this->requires_certification = new DbField(
            $this, // Table
            'x_requires_certification', // Variable name
            'requires_certification', // Name
            '"requires_certification"', // Expression
            'CAST("requires_certification" AS varchar(255))', // Basic search expression
            11, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"requires_certification"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->requires_certification->InputTextType = "text";
        $this->requires_certification->Raw = true;
        $this->requires_certification->setDataType(DataType::BOOLEAN);
        $this->requires_certification->Lookup = new Lookup($this->requires_certification, 'equipment_types', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->requires_certification->OptionCount = 2;
        $this->requires_certification->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['requires_certification'] = &$this->requires_certification;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "equipment_types";
    }
    ...
    public function insert(&$rs)
    {
        ...
        try {
            ...
            $result = $conn->executeQuery(
                $queryBuilder->getSQL() . " RETURNING type_id",
        ...
        if ($result) {
            $this->type_id->setDbValue($result);
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['type_id']) && !EmptyValue($this->type_id->CurrentValue)) {
                $rs['type_id'] = $this->type_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->maintenance_interval->DbValue = $row['maintenance_interval'];
        $this->requires_certification->DbValue = (ConvertToBool($row['requires_certification']) ? "1" : "0");
    ...
    {
        $keys = [];
        $val = $current ? $this->type_id->CurrentValue : $this->type_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->type_id->CurrentValue = $keys[0];
            } else {
                $this->type_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('type_id', $row) ? $row['type_id'] : null;
        } else {
            $val = !EmptyValue($this->type_id->OldValue) && !$current ? $this->type_id->OldValue : $this->type_id->CurrentValue;
        }
        if (!is_numeric($val)) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("EquipmentTypesList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "EquipmentTypesView" => $Language->phrase("View"),
            "EquipmentTypesEdit" => $Language->phrase("Edit"),
            "EquipmentTypesAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "EquipmentTypesView",
            Config("API_ADD_ACTION") => "EquipmentTypesAdd",
            Config("API_EDIT_ACTION") => "EquipmentTypesEdit",
            Config("API_DELETE_ACTION") => "EquipmentTypesDelete",
            Config("API_LIST_ACTION") => "EquipmentTypesList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("EquipmentTypesView", $parm);
        } else {
            $url = $this->keyUrl("EquipmentTypesView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "EquipmentTypesAdd?" . $parm;
        } else {
            $url = "EquipmentTypesAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("EquipmentTypesEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("EquipmentTypesList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("EquipmentTypesAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("EquipmentTypesList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("EquipmentTypesDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"type_id\":" . VarToJson($this->type_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        if ($this->type_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->type_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("type_id") ?? Route("type_id")) !== null) {
                $arKeys[] = $keyValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->maintenance_interval);
                    $doc->exportCaption($this->requires_certification);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->maintenance_interval);
                        $doc->exportField($this->requires_certification);
