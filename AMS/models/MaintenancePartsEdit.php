 ...
 * Page class
 */
class MaintenancePartsEdit extends MaintenanceParts
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MaintenancePartsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "MaintenancePartsEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("part_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->part_id->setQueryStringValue($keyValue);
                $this->part_id->setOldValue($this->part_id->QueryStringValue);
            } elseif (Post("part_id") !== null) {
                $this->part_id->setFormValue(Post("part_id"));
                $this->part_id->setOldValue($this->part_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("part_id") ?? Route("part_id")) !== null) {
                    ...
                    $this->part_id->CurrentValue = null;
                }
            }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->part_id->IsDetailKey) {
            $this->part_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->part_id->CurrentValue = $this->part_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // part_id
            $this->part_id->setupEditAttributes();
            $this->part_id->EditValue = $this->part_id->CurrentValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->part_id->Visible && $this->part_id->Required) {
                if (!$this->part_id->IsDetailKey && EmptyValue($this->part_id->FormValue)) {
                    $this->part_id->addErrorMessage(str_replace("%s", $this->part_id->caption(), $this->part_id->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // part_name
        $this->part_name->setDbValueDef($rsnew, $this->part_name->CurrentValue, $this->part_name->ReadOnly);

        // quantity
        $this->quantity->setDbValueDef($rsnew, $this->quantity->CurrentValue, $this->quantity->ReadOnly);

        // unit_cost
        $this->unit_cost->setDbValueDef($rsnew, $this->unit_cost->CurrentValue, $this->unit_cost->ReadOnly);

        // total_cost
        $this->total_cost->setDbValueDef($rsnew, $this->total_cost->CurrentValue, $this->total_cost->ReadOnly);

        // supplier
        $this->supplier->setDbValueDef($rsnew, $this->supplier->CurrentValue, $this->supplier->ReadOnly);

        // warranty_period
        $this->warranty_period->setDbValueDef($rsnew, $this->warranty_period->CurrentValue, $this->warranty_period->ReadOnly);
