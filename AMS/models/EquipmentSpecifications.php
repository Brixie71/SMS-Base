...
/**
 * Table class for equipment_specifications
 */
class EquipmentSpecifications extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "equipment_specifications";
        ...
        // spec_id
        $this->spec_id = new DbField(
            $this, // Table
            'x_spec_id', // Variable name
            'spec_id', // Name
            '"spec_id"', // Expression
            'CAST("spec_id" AS varchar(255))', // Basic search expression
            3, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"spec_id"', // Virtual expression
            false, // Is virtual
        ...
        $this->spec_id->InputTextType = "text";
        $this->spec_id->Raw = true;
        $this->spec_id->IsAutoIncrement = true; // Autoincrement field
        $this->spec_id->IsPrimaryKey = true; // Primary key field
        $this->spec_id->Nullable = false; // NOT NULL field
        $this->spec_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->spec_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->spec_type_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        $this->unit_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "equipment_specifications";
    }
    ...
    public function insert(&$rs)
    {
        ...
        try {
            ...
            $result = $conn->executeQuery(
                $queryBuilder->getSQL() . " RETURNING spec_id",
        ...
        if ($result) {
            $this->spec_id->setDbValue($result);
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['spec_id']) && !EmptyValue($this->spec_id->CurrentValue)) {
                $rs['spec_id'] = $this->spec_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('spec_id', $rs)) {
                AddFilter($where, QuotedName('spec_id', $this->Dbid) . '=' . QuotedValue($rs['spec_id'], $this->spec_id->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->spec_id->CurrentValue : $this->spec_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->spec_id->CurrentValue = $keys[0];
            } else {
                $this->spec_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('spec_id', $row) ? $row['spec_id'] : null;
        } else {
            $val = !EmptyValue($this->spec_id->OldValue) && !$current ? $this->spec_id->OldValue : $this->spec_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@spec_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("EquipmentSpecificationsList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "EquipmentSpecificationsView" => $Language->phrase("View"),
            "EquipmentSpecificationsEdit" => $Language->phrase("Edit"),
            "EquipmentSpecificationsAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    {
        return "EquipmentSpecificationsList";
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "EquipmentSpecificationsView",
            Config("API_ADD_ACTION") => "EquipmentSpecificationsAdd",
            Config("API_EDIT_ACTION") => "EquipmentSpecificationsEdit",
            Config("API_DELETE_ACTION") => "EquipmentSpecificationsDelete",
            Config("API_LIST_ACTION") => "EquipmentSpecificationsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("EquipmentSpecificationsView", $parm);
        } else {
            $url = $this->keyUrl("EquipmentSpecificationsView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "EquipmentSpecificationsAdd?" . $parm;
        } else {
            $url = "EquipmentSpecificationsAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("EquipmentSpecificationsEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("EquipmentSpecificationsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("EquipmentSpecificationsAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("EquipmentSpecificationsList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("EquipmentSpecificationsDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"spec_id\":" . VarToJson($this->spec_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->spec_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->spec_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("spec_id") ?? Route("spec_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    {
        global $Response;
        $listPage = "EquipmentSpecificationsList";
