...
class Notifications extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->timestamp->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        $this->target->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        $this->link = new DbField(
            $this, // Table
            'x_link', // Variable name
            'link', // Name
            '"link"', // Expression
            '"link"', // Basic search expression
            200, // Type
            255, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"link"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->link->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['link'] = &$this->link;

        // from_system
        $this->from_system = new DbField(
            $this, // Table
            'x_from_system', // Variable name
            'from_system', // Name
            '"from_system"', // Expression
            '"from_system"', // Basic search expression
            200, // Type
            20, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"from_system"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->from_system->InputTextType = "text";
        $this->from_system->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['from_system'] = &$this->from_system;

        // is_read
        $this->is_read = new DbField(
            $this, // Table
            'x_is_read', // Variable name
            'is_read', // Name
            '"is_read"', // Expression
            'CAST("is_read" AS varchar(255))', // Basic search expression
            11, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"is_read"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->is_read->InputTextType = "text";
        $this->is_read->Raw = true;
        $this->is_read->setDataType(DataType::BOOLEAN);
        $this->is_read->Lookup = new Lookup($this->is_read, 'notifications', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->is_read->OptionCount = 2;
        $this->is_read->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['is_read'] = &$this->is_read;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "notifications";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->from_system->DbValue = $row['from_system'];
        $this->is_read->DbValue = (ConvertToBool($row['is_read']) ? "1" : "0");
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("NotificationsList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "NotificationsView" => $Language->phrase("View"),
            "NotificationsEdit" => $Language->phrase("Edit"),
            "NotificationsAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "NotificationsView",
            Config("API_ADD_ACTION") => "NotificationsAdd",
            Config("API_EDIT_ACTION") => "NotificationsEdit",
            Config("API_DELETE_ACTION") => "NotificationsDelete",
            Config("API_LIST_ACTION") => "NotificationsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("NotificationsView", $parm);
        } else {
            $url = $this->keyUrl("NotificationsView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("NotificationsEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("NotificationsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("NotificationsAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("NotificationsList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("NotificationsDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->from_system->EditValue = $this->from_system->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->from_system);
                    $doc->exportCaption($this->is_read);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->from_system);
                        $doc->exportField($this->is_read);
