...
/**
 * Table class for team_members
 */
class TeamMembers extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->role->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['role'] = &$this->role;

        // assigned_date
        $this->assigned_date = new DbField(
            $this, // Table
            'x_assigned_date', // Variable name
            'assigned_date', // Name
            '"assigned_date"', // Expression
            CastDateFieldForLike("\"assigned_date\"", 0, "philtower_ams"), // Basic search expression
            133, // Type
            0, // Size
            0, // Date/Time format
            false, // Is upload field
            '"assigned_date"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->assigned_date->InputTextType = "text";
        $this->assigned_date->Raw = true;
        $this->assigned_date->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
        $this->assigned_date->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['assigned_date'] = &$this->assigned_date;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "team_members";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->assigned_date->DbValue = $row['assigned_date'];
    ...
    {
        return "\"team_id\" = @team_id@ AND \"user_id\" = @user_id@";
    }
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 2) {
            ...
            if ($current) {
                $this->user_id->CurrentValue = $keys[1];
            } else {
                $this->user_id->OldValue = $keys[1];
            }
        }
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("TeamMembersList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "TeamMembersView" => $Language->phrase("View"),
            "TeamMembersEdit" => $Language->phrase("Edit"),
            "TeamMembersAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "TeamMembersView",
            Config("API_ADD_ACTION") => "TeamMembersAdd",
            Config("API_EDIT_ACTION") => "TeamMembersEdit",
            Config("API_DELETE_ACTION") => "TeamMembersDelete",
            Config("API_LIST_ACTION") => "TeamMembersList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TeamMembersView", $parm);
        } else {
            $url = $this->keyUrl("TeamMembersView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TeamMembersEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TeamMembersList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TeamMembersAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TeamMembersList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("TeamMembersDelete", $parm);
        }
    }
    ...
    public function keyUrl($url, $parm = "")
    {
        ...
        if ($this->user_id->CurrentValue !== null) {
            $url .= $this->RouteCompositeKeySeparator . $this->encodeKeyValue($this->user_id->CurrentValue);
        } else {
            return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
        }
        if ($parm != "") {
    ...
    public function renderEditRow()
    {
        ...
        $this->assigned_date->EditValue = FormatDateTime($this->assigned_date->CurrentValue, $this->assigned_date->formatPattern());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->assigned_date);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->assigned_date);
