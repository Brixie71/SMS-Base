 ...
 * Page class
 */
class ManufacturersEdit extends Manufacturers
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ManufacturersEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ManufacturersEdit";
    ...
    public function setVisibility()
    {
        ...
        $this->support_contact->setVisibility();
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("manufacturer_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->manufacturer_id->setQueryStringValue($keyValue);
                $this->manufacturer_id->setOldValue($this->manufacturer_id->QueryStringValue);
            } elseif (Post("manufacturer_id") !== null) {
                $this->manufacturer_id->setFormValue(Post("manufacturer_id"));
                $this->manufacturer_id->setOldValue($this->manufacturer_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("manufacturer_id") ?? Route("manufacturer_id")) !== null) {
                    $this->manufacturer_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->manufacturer_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ManufacturersList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "ManufacturersList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ManufacturersList") {
                            ...
                            $returnUrl = "ManufacturersList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->manufacturer_id->IsDetailKey) {
            $this->manufacturer_id->setFormValue($val);
        }
        ...
        // Check field name 'support_contact' first before field var 'x_support_contact'
        $val = $CurrentForm->hasValue("support_contact") ? $CurrentForm->getValue("support_contact") : $CurrentForm->getValue("x_support_contact");
        if (!$this->support_contact->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->support_contact->Visible = false; // Disable update for API request
            } else {
                $this->support_contact->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->support_contact->CurrentValue = $this->support_contact->FormValue;
    ...
    public function renderRow()
    {
        ...
        // support_contact
        $this->support_contact->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->support_contact->EditValue = HtmlEncode($this->support_contact->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->support_contact->Visible && $this->support_contact->Required) {
                if (!$this->support_contact->IsDetailKey && EmptyValue($this->support_contact->FormValue)) {
                    $this->support_contact->addErrorMessage(str_replace("%s", $this->support_contact->caption(), $this->support_contact->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // support_contact
        $this->support_contact->setDbValueDef($rsnew, $this->support_contact->CurrentValue, $this->support_contact->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['support_contact'])) { // support_contact
            $this->support_contact->CurrentValue = $row['support_contact'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ManufacturersList"), "", $this->TableVar, true);
        $pageId = "edit";
