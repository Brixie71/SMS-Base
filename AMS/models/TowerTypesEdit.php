 ...
 * Page class
 */
class TowerTypesEdit extends TowerTypes
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TowerTypesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "TowerTypesEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("TowerTypesList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "TowerTypesList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "TowerTypesList") {
                            ...
                            $returnUrl = "TowerTypesList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'height_limit' first before field var 'x_height_limit'
        $val = $CurrentForm->hasValue("height_limit") ? $CurrentForm->getValue("height_limit") : $CurrentForm->getValue("x_height_limit");
        if (!$this->height_limit->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->height_limit->Visible = false; // Disable update for API request
            } else {
                $this->height_limit->setFormValue($val, true, $validate);
            }
        }

        // Check field name 'equipment_limit' first before field var 'x_equipment_limit'
        $val = $CurrentForm->hasValue("equipment_limit") ? $CurrentForm->getValue("equipment_limit") : $CurrentForm->getValue("x_equipment_limit");
        if (!$this->equipment_limit->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->equipment_limit->Visible = false; // Disable update for API request
            } else {
                $this->equipment_limit->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->height_limit->CurrentValue = $this->height_limit->FormValue;
        $this->equipment_limit->CurrentValue = $this->equipment_limit->FormValue;
    ...
    public function renderRow()
    {
        ...
        // height_limit
        $this->height_limit->RowCssClass = "row";

        // equipment_limit
        $this->equipment_limit->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // height_limit
            $this->height_limit->setupEditAttributes();
            $this->height_limit->EditValue = $this->height_limit->CurrentValue;
            $this->height_limit->PlaceHolder = RemoveHtml($this->height_limit->caption());
            if (strval($this->height_limit->EditValue) != "" && is_numeric($this->height_limit->EditValue)) {
                $this->height_limit->EditValue = FormatNumber($this->height_limit->EditValue, null);
            }

            // equipment_limit
            $this->equipment_limit->setupEditAttributes();
            $this->equipment_limit->EditValue = $this->equipment_limit->CurrentValue;
            $this->equipment_limit->PlaceHolder = RemoveHtml($this->equipment_limit->caption());
            if (strval($this->equipment_limit->EditValue) != "" && is_numeric($this->equipment_limit->EditValue)) {
                $this->equipment_limit->EditValue = FormatNumber($this->equipment_limit->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->height_limit->Visible && $this->height_limit->Required) {
                if (!$this->height_limit->IsDetailKey && EmptyValue($this->height_limit->FormValue)) {
                    $this->height_limit->addErrorMessage(str_replace("%s", $this->height_limit->caption(), $this->height_limit->RequiredErrorMessage));
                }
            }
            if (!CheckNumber($this->height_limit->FormValue)) {
                $this->height_limit->addErrorMessage($this->height_limit->getErrorMessage(false));
            }
            if ($this->equipment_limit->Visible && $this->equipment_limit->Required) {
                if (!$this->equipment_limit->IsDetailKey && EmptyValue($this->equipment_limit->FormValue)) {
                    $this->equipment_limit->addErrorMessage(str_replace("%s", $this->equipment_limit->caption(), $this->equipment_limit->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->equipment_limit->FormValue)) {
                $this->equipment_limit->addErrorMessage($this->equipment_limit->getErrorMessage(false));
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // height_limit
        $this->height_limit->setDbValueDef($rsnew, $this->height_limit->CurrentValue, $this->height_limit->ReadOnly);

        // equipment_limit
        $this->equipment_limit->setDbValueDef($rsnew, $this->equipment_limit->CurrentValue, $this->equipment_limit->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['height_limit'])) { // height_limit
            $this->height_limit->CurrentValue = $row['height_limit'];
        }
        if (isset($row['equipment_limit'])) { // equipment_limit
            $this->equipment_limit->CurrentValue = $row['equipment_limit'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("TowerTypesList"), "", $this->TableVar, true);
        $pageId = "edit";
