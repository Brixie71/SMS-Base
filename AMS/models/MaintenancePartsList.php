 ...
 * Page class
 */
class MaintenancePartsList extends MaintenanceParts
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MaintenancePartsList";
    ...
    public $FormName = "fmaintenance_partslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "MaintenancePartsList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "MaintenancePartsAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "MaintenancePartsDelete";
        $this->MultiUpdateUrl = "MaintenancePartsUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fmaintenance_partsgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fmaintenance_partssrch");
        }
        $filterList = Concat($filterList, $this->part_id->AdvancedSearch->toJson(), ","); // Field part_id
        $filterList = Concat($filterList, $this->log_id->AdvancedSearch->toJson(), ","); // Field log_id
        $filterList = Concat($filterList, $this->part_name->AdvancedSearch->toJson(), ","); // Field part_name
        $filterList = Concat($filterList, $this->quantity->AdvancedSearch->toJson(), ","); // Field quantity
        $filterList = Concat($filterList, $this->unit_cost->AdvancedSearch->toJson(), ","); // Field unit_cost
        $filterList = Concat($filterList, $this->total_cost->AdvancedSearch->toJson(), ","); // Field total_cost
        $filterList = Concat($filterList, $this->supplier->AdvancedSearch->toJson(), ","); // Field supplier
        $filterList = Concat($filterList, $this->warranty_period->AdvancedSearch->toJson(), ","); // Field warranty_period
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fmaintenance_partssrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field part_id
        $this->part_id->AdvancedSearch->SearchValue = @$filter["x_part_id"];
        $this->part_id->AdvancedSearch->SearchOperator = @$filter["z_part_id"];
        $this->part_id->AdvancedSearch->SearchCondition = @$filter["v_part_id"];
        $this->part_id->AdvancedSearch->SearchValue2 = @$filter["y_part_id"];
        $this->part_id->AdvancedSearch->SearchOperator2 = @$filter["w_part_id"];
        $this->part_id->AdvancedSearch->save();
        ...
        $this->warranty_period->AdvancedSearch->SearchValue = @$filter["x_warranty_period"];
        $this->warranty_period->AdvancedSearch->SearchOperator = @$filter["z_warranty_period"];
        $this->warranty_period->AdvancedSearch->SearchCondition = @$filter["v_warranty_period"];
        $this->warranty_period->AdvancedSearch->SearchValue2 = @$filter["y_warranty_period"];
        $this->warranty_period->AdvancedSearch->SearchOperator2 = @$filter["w_warranty_period"];
        $this->warranty_period->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->part_name;
        $searchFlds[] = &$this->supplier;
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->part_id->setSort("");
                $this->log_id->setSort("");
                $this->part_name->setSort("");
                $this->quantity->setSort("");
                $this->unit_cost->setSort("");
                $this->total_cost->setSort("");
                $this->supplier->setSort("");
                $this->warranty_period->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"maintenance_parts\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"maintenance_parts\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"maintenance_parts\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fmaintenance_partslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fmaintenance_partslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->part_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"maintenance_parts\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "part_id");
            $this->createColumnOption($option, "log_id");
            $this->createColumnOption($option, "part_name");
            $this->createColumnOption($option, "quantity");
            $this->createColumnOption($option, "unit_cost");
            $this->createColumnOption($option, "total_cost");
            $this->createColumnOption($option, "supplier");
            $this->createColumnOption($option, "warranty_period");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmaintenance_partssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmaintenance_partssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmaintenance_partslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_maintenance_parts", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_maintenance_parts",
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // part_id
            $this->part_id->HrefValue = "";
            $this->part_id->TooltipValue = "";
            ...
            $this->warranty_period->TooltipValue = "";
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmaintenance_partssrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
