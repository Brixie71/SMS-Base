 ...
 * Page class
 */
class TeamMembersEdit extends TeamMembers
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TeamMembersEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "TeamMembersEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("team_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->team_id->setQueryStringValue($keyValue);
                $this->team_id->setOldValue($this->team_id->QueryStringValue);
            } elseif (Post("team_id") !== null) {
                $this->team_id->setFormValue(Post("team_id"));
                $this->team_id->setOldValue($this->team_id->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            if (($keyValue = Get("user_id") ?? Key(1) ?? Route(3)) !== null) {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("team_id") ?? Route("team_id")) !== null) {
                    $this->team_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("TeamMembersList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "TeamMembersList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "TeamMembersList") {
                            ...
                            $returnUrl = "TeamMembersList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_team_id")) {
            $this->team_id->setOldValue($CurrentForm->getValue("o_team_id"));
        }
        ...
        // Check field name 'assigned_date' first before field var 'x_assigned_date'
        $val = $CurrentForm->hasValue("assigned_date") ? $CurrentForm->getValue("assigned_date") : $CurrentForm->getValue("x_assigned_date");
        if (!$this->assigned_date->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->assigned_date->Visible = false; // Disable update for API request
            } else {
                $this->assigned_date->setFormValue($val, true, $validate);
            }
            $this->assigned_date->CurrentValue = UnFormatDateTime($this->assigned_date->CurrentValue, $this->assigned_date->formatPattern());
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->assigned_date->CurrentValue = $this->assigned_date->FormValue;
        $this->assigned_date->CurrentValue = UnFormatDateTime($this->assigned_date->CurrentValue, $this->assigned_date->formatPattern());
    ...
    public function renderRow()
    {
        ...
        // assigned_date
        $this->assigned_date->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // assigned_date
            $this->assigned_date->setupEditAttributes();
            $this->assigned_date->EditValue = HtmlEncode(FormatDateTime($this->assigned_date->CurrentValue, $this->assigned_date->formatPattern()));
            $this->assigned_date->PlaceHolder = RemoveHtml($this->assigned_date->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->assigned_date->Visible && $this->assigned_date->Required) {
                if (!$this->assigned_date->IsDetailKey && EmptyValue($this->assigned_date->FormValue)) {
                    $this->assigned_date->addErrorMessage(str_replace("%s", $this->assigned_date->caption(), $this->assigned_date->RequiredErrorMessage));
                }
            }
            if (!CheckDate($this->assigned_date->FormValue, $this->assigned_date->formatPattern())) {
                $this->assigned_date->addErrorMessage($this->assigned_date->getErrorMessage(false));
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // assigned_date
        $this->assigned_date->setDbValueDef($rsnew, UnFormatDateTime($this->assigned_date->CurrentValue, $this->assigned_date->formatPattern()), $this->assigned_date->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['role'])) { // role
            $this->role->CurrentValue = $row['role'];
        }
        if (isset($row['assigned_date'])) { // assigned_date
            $this->assigned_date->CurrentValue = $row['assigned_date'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("TeamMembersList"), "", $this->TableVar, true);
        $pageId = "edit";
