...
class StatusTypes extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // is_operational
        $this->is_operational = new DbField(
            $this, // Table
            'x_is_operational', // Variable name
            'is_operational', // Name
            '"is_operational"', // Expression
            'CAST("is_operational" AS varchar(255))', // Basic search expression
            11, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"is_operational"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->is_operational->InputTextType = "text";
        $this->is_operational->Raw = true;
        $this->is_operational->setDataType(DataType::BOOLEAN);
        $this->is_operational->Lookup = new Lookup($this->is_operational, 'status_types', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->is_operational->OptionCount = 2;
        $this->is_operational->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['is_operational'] = &$this->is_operational;
        ...
        // sequence_no
        $this->sequence_no = new DbField(
            $this, // Table
            'x_sequence_no', // Variable name
            'sequence_no', // Name
            '"sequence_no"', // Expression
            'CAST("sequence_no" AS varchar(255))', // Basic search expression
            3, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"sequence_no"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->sequence_no->InputTextType = "text";
        $this->sequence_no->Raw = true;
        $this->sequence_no->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->sequence_no->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
    ...
    public function insert(&$rs)
    {
        ...
        try {
            ...
            $result = $conn->executeQuery(
                $queryBuilder->getSQL() . " RETURNING status_id",
        ...
        if ($result) {
            $this->status_id->setDbValue($result);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->is_operational->DbValue = (ConvertToBool($row['is_operational']) ? "1" : "0");
        $this->color_code->DbValue = $row['color_code'];
        $this->icon_class->DbValue = $row['icon_class'];
        $this->sequence_no->DbValue = $row['sequence_no'];
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->is_operational);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->is_operational);
