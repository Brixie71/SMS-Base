 ...
 * Page class
 */
class SpecificationTypesEdit extends SpecificationTypes
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SpecificationTypesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "SpecificationTypesEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("spec_type_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->spec_type_id->setQueryStringValue($keyValue);
                $this->spec_type_id->setOldValue($this->spec_type_id->QueryStringValue);
            } elseif (Post("spec_type_id") !== null) {
                $this->spec_type_id->setFormValue(Post("spec_type_id"));
                $this->spec_type_id->setOldValue($this->spec_type_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("spec_type_id") ?? Route("spec_type_id")) !== null) {
                    $this->spec_type_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->spec_type_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("SpecificationTypesList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "SpecificationTypesList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "SpecificationTypesList") {
                            ...
                            $returnUrl = "SpecificationTypesList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->spec_type_id->IsDetailKey) {
            $this->spec_type_id->setFormValue($val);
        }
        ...
        // Check field name 'unit_category_id' first before field var 'x_unit_category_id'
        $val = $CurrentForm->hasValue("unit_category_id") ? $CurrentForm->getValue("unit_category_id") : $CurrentForm->getValue("x_unit_category_id");
        if (!$this->unit_category_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->unit_category_id->Visible = false; // Disable update for API request
            } else {
                $this->unit_category_id->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->unit_category_id->CurrentValue = $this->unit_category_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // unit_category_id
        $this->unit_category_id->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->unit_category_id->Visible && $this->unit_category_id->Required) {
                if (!$this->unit_category_id->IsDetailKey && EmptyValue($this->unit_category_id->FormValue)) {
                    $this->unit_category_id->addErrorMessage(str_replace("%s", $this->unit_category_id->caption(), $this->unit_category_id->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->unit_category_id->FormValue)) {
                $this->unit_category_id->addErrorMessage($this->unit_category_id->getErrorMessage(false));
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // unit_category_id
        $this->unit_category_id->setDbValueDef($rsnew, $this->unit_category_id->CurrentValue, $this->unit_category_id->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['unit_category_id'])) { // unit_category_id
            $this->unit_category_id->CurrentValue = $row['unit_category_id'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("SpecificationTypesList"), "", $this->TableVar, true);
        $pageId = "edit";
