...
class AggregatedAuditLogsGrid extends AggregatedAuditLogs
{
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x__action") &&
            $CurrentForm->hasValue("o__action") &&
            $this->_action->CurrentValue != $this->_action->DefaultValue &&
            !($this->_action->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->_action->CurrentValue == $this->_action->getSessionValue())
        ) {
            return false;
        }
        if (
        ...
        if (
            $CurrentForm->hasValue("x_action_count") &&
            $CurrentForm->hasValue("o_action_count") &&
            $this->action_count->CurrentValue != $this->action_count->DefaultValue &&
            !($this->action_count->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->action_count->CurrentValue == $this->action_count->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o__action")) {
            $this->_action->setOldValue($CurrentForm->getValue("o__action"));
        }
        ...
        // Check field name 'action_count' first before field var 'x_action_count'
        $val = $CurrentForm->hasValue("action_count") ? $CurrentForm->getValue("action_count") : $CurrentForm->getValue("x_action_count");
        if (!$this->action_count->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->action_count->Visible = false; // Disable update for API request
            } else {
                $this->action_count->setFormValue($val, true, $validate);
            }
        }
        if ($CurrentForm->hasValue("o_action_count")) {
            $this->action_count->setOldValue($CurrentForm->getValue("o_action_count"));
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->action_count->CurrentValue = $this->action_count->FormValue;
    }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->action_count->Visible && $this->action_count->Required) {
                if (!$this->action_count->IsDetailKey && EmptyValue($this->action_count->FormValue)) {
                    $this->action_count->addErrorMessage(str_replace("%s", $this->action_count->caption(), $this->action_count->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->action_count->FormValue)) {
                $this->action_count->addErrorMessage($this->action_count->getErrorMessage(false));
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // action_count
        $this->action_count->setDbValueDef($rsnew, $this->action_count->CurrentValue, $this->action_count->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['action_count'])) { // action_count
            $this->action_count->CurrentValue = $row['action_count'];
        }
    }
    ...
    protected function getAddRow()
    {
        ...
        // action_count
        $this->action_count->setDbValueDef($rsnew, $this->action_count->CurrentValue, false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['action_count'])) { // action_count
            $this->action_count->setFormValue($row['action_count']);
        }
    }
