...
/**
 * Table class for tower_types
 */
class TowerTypes extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // height_limit
        $this->height_limit = new DbField(
            $this, // Table
            'x_height_limit', // Variable name
            'height_limit', // Name
            '"height_limit"', // Expression
            'CAST("height_limit" AS varchar(255))', // Basic search expression
            14, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"height_limit"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->height_limit->InputTextType = "text";
        $this->height_limit->Raw = true;
        $this->height_limit->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
        $this->height_limit->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['height_limit'] = &$this->height_limit;

        // equipment_limit
        $this->equipment_limit = new DbField(
            $this, // Table
            'x_equipment_limit', // Variable name
            'equipment_limit', // Name
            '"equipment_limit"', // Expression
            'CAST("equipment_limit" AS varchar(255))', // Basic search expression
            3, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"equipment_limit"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->equipment_limit->InputTextType = "text";
        $this->equipment_limit->Raw = true;
        $this->equipment_limit->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->equipment_limit->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['equipment_limit'] = &$this->equipment_limit;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "tower_types";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->height_limit->DbValue = $row['height_limit'];
        $this->equipment_limit->DbValue = $row['equipment_limit'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("TowerTypesList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "TowerTypesView" => $Language->phrase("View"),
            "TowerTypesEdit" => $Language->phrase("Edit"),
            "TowerTypesAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "TowerTypesView",
            Config("API_ADD_ACTION") => "TowerTypesAdd",
            Config("API_EDIT_ACTION") => "TowerTypesEdit",
            Config("API_DELETE_ACTION") => "TowerTypesDelete",
            Config("API_LIST_ACTION") => "TowerTypesList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TowerTypesView", $parm);
        } else {
            $url = $this->keyUrl("TowerTypesView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TowerTypesEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TowerTypesList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TowerTypesAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TowerTypesList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("TowerTypesDelete", $parm);
        }
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->height_limit);
                    $doc->exportCaption($this->equipment_limit);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->height_limit);
                        $doc->exportField($this->equipment_limit);
