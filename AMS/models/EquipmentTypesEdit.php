 ...
 * Page class
 */
class EquipmentTypesEdit extends EquipmentTypes
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EquipmentTypesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "EquipmentTypesEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("type_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->type_id->setQueryStringValue($keyValue);
                $this->type_id->setOldValue($this->type_id->QueryStringValue);
            } elseif (Post("type_id") !== null) {
                $this->type_id->setFormValue(Post("type_id"));
                $this->type_id->setOldValue($this->type_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("type_id") ?? Route("type_id")) !== null) {
                    $this->type_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("EquipmentTypesList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "EquipmentTypesList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "EquipmentTypesList") {
                            ...
                            $returnUrl = "EquipmentTypesList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'maintenance_interval' first before field var 'x_maintenance_interval'
        $val = $CurrentForm->hasValue("maintenance_interval") ? $CurrentForm->getValue("maintenance_interval") : $CurrentForm->getValue("x_maintenance_interval");
        if (!$this->maintenance_interval->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->maintenance_interval->Visible = false; // Disable update for API request
            } else {
                $this->maintenance_interval->setFormValue($val, true, $validate);
            }
        }

        // Check field name 'requires_certification' first before field var 'x_requires_certification'
        $val = $CurrentForm->hasValue("requires_certification") ? $CurrentForm->getValue("requires_certification") : $CurrentForm->getValue("x_requires_certification");
        if (!$this->requires_certification->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->requires_certification->Visible = false; // Disable update for API request
            } else {
                $this->requires_certification->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->maintenance_interval->CurrentValue = $this->maintenance_interval->FormValue;
        $this->requires_certification->CurrentValue = $this->requires_certification->FormValue;
    ...
    public function renderRow()
    {
        ...
        $this->maintenance_interval->RowCssClass = "row";

        // requires_certification
        $this->requires_certification->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // maintenance_interval
            $this->maintenance_interval->setupEditAttributes();
            $this->maintenance_interval->EditValue = $this->maintenance_interval->CurrentValue;
            $this->maintenance_interval->PlaceHolder = RemoveHtml($this->maintenance_interval->caption());
            if (strval($this->maintenance_interval->EditValue) != "" && is_numeric($this->maintenance_interval->EditValue)) {
                $this->maintenance_interval->EditValue = FormatNumber($this->maintenance_interval->EditValue, null);
            }

            // requires_certification
            $this->requires_certification->EditValue = $this->requires_certification->options(false);
            $this->requires_certification->PlaceHolder = RemoveHtml($this->requires_certification->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->maintenance_interval->Visible && $this->maintenance_interval->Required) {
                if (!$this->maintenance_interval->IsDetailKey && EmptyValue($this->maintenance_interval->FormValue)) {
                    $this->maintenance_interval->addErrorMessage(str_replace("%s", $this->maintenance_interval->caption(), $this->maintenance_interval->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->maintenance_interval->FormValue)) {
                $this->maintenance_interval->addErrorMessage($this->maintenance_interval->getErrorMessage(false));
            }
            if ($this->requires_certification->Visible && $this->requires_certification->Required) {
                if ($this->requires_certification->FormValue == "") {
                    $this->requires_certification->addErrorMessage(str_replace("%s", $this->requires_certification->caption(), $this->requires_certification->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // maintenance_interval
        $this->maintenance_interval->setDbValueDef($rsnew, $this->maintenance_interval->CurrentValue, $this->maintenance_interval->ReadOnly);

        // requires_certification
        $tmpBool = $this->requires_certification->CurrentValue;
        if ($tmpBool != "1" && $tmpBool != "0") {
            $tmpBool = !empty($tmpBool) ? "1" : "0";
        }
        $this->requires_certification->setDbValueDef($rsnew, $tmpBool, $this->requires_certification->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['maintenance_interval'])) { // maintenance_interval
            $this->maintenance_interval->CurrentValue = $row['maintenance_interval'];
        }
        if (isset($row['requires_certification'])) { // requires_certification
            $this->requires_certification->CurrentValue = $row['requires_certification'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("EquipmentTypesList"), "", $this->TableVar, true);
        $pageId = "edit";
