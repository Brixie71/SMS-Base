...
/**
 * Table class for manufacturers
 */
class Manufacturers extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "manufacturers";
        ...
        $this->manufacturer_id->IsAutoIncrement = true; // Autoincrement field
        $this->manufacturer_id->IsPrimaryKey = true; // Primary key field
        $this->manufacturer_id->Nullable = false; // NOT NULL field
        $this->manufacturer_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->manufacturer_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->contact_person->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        // support_contact
        $this->support_contact = new DbField(
            $this, // Table
            'x_support_contact', // Variable name
            'support_contact', // Name
            '"support_contact"', // Expression
            '"support_contact"', // Basic search expression
            201, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"support_contact"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->support_contact->InputTextType = "text";
        $this->support_contact->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['support_contact'] = &$this->support_contact;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "manufacturers";
    }
    ...
    public function insert(&$rs)
    {
        ...
        try {
            ...
            $result = $conn->executeQuery(
                $queryBuilder->getSQL() . " RETURNING manufacturer_id",
        ...
        if ($result) {
            $this->manufacturer_id->setDbValue($result);
            $rs['manufacturer_id'] = $this->manufacturer_id->DbValue;
        }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['manufacturer_id']) && !EmptyValue($this->manufacturer_id->CurrentValue)) {
                $rs['manufacturer_id'] = $this->manufacturer_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('manufacturer_id', $rs)) {
                AddFilter($where, QuotedName('manufacturer_id', $this->Dbid) . '=' . QuotedValue($rs['manufacturer_id'], $this->manufacturer_id->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->support_contact->DbValue = $row['support_contact'];
    ...
    {
        $keys = [];
        $val = $current ? $this->manufacturer_id->CurrentValue : $this->manufacturer_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->manufacturer_id->CurrentValue = $keys[0];
            } else {
                $this->manufacturer_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('manufacturer_id', $row) ? $row['manufacturer_id'] : null;
        } else {
            $val = !EmptyValue($this->manufacturer_id->OldValue) && !$current ? $this->manufacturer_id->OldValue : $this->manufacturer_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@manufacturer_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ManufacturersList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ManufacturersView" => $Language->phrase("View"),
            "ManufacturersEdit" => $Language->phrase("Edit"),
            "ManufacturersAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ManufacturersView",
            Config("API_ADD_ACTION") => "ManufacturersAdd",
            Config("API_EDIT_ACTION") => "ManufacturersEdit",
            Config("API_DELETE_ACTION") => "ManufacturersDelete",
            Config("API_LIST_ACTION") => "ManufacturersList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ManufacturersView", $parm);
        } else {
            $url = $this->keyUrl("ManufacturersView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ManufacturersAdd?" . $parm;
        } else {
            $url = "ManufacturersAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ManufacturersEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ManufacturersList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ManufacturersAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ManufacturersList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ManufacturersDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"manufacturer_id\":" . VarToJson($this->manufacturer_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->manufacturer_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->manufacturer_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("manufacturer_id") ?? Route("manufacturer_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->manufacturer_id->CurrentValue = $key;
            } else {
                $this->manufacturer_id->OldValue = $key;
            }
    ...
    public function renderListRow()
    {
        ...
        // support_contact
        $this->support_contact->ViewValue = $this->support_contact->CurrentValue;
        ...
        // support_contact
        $this->support_contact->HrefValue = "";
        $this->support_contact->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // support_contact
        $this->support_contact->setupEditAttributes();
        $this->support_contact->EditValue = $this->support_contact->CurrentValue;
        $this->support_contact->PlaceHolder = RemoveHtml($this->support_contact->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->support_contact);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->support_contact);
