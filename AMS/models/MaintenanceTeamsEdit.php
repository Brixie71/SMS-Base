 ...
 * Page class
 */
class MaintenanceTeamsEdit extends MaintenanceTeams
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MaintenanceTeamsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "MaintenanceTeamsEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("MaintenanceTeamsList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "MaintenanceTeamsList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "MaintenanceTeamsList") {
                            ...
                            $returnUrl = "MaintenanceTeamsList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'supervisor_id' first before field var 'x_supervisor_id'
        $val = $CurrentForm->hasValue("supervisor_id") ? $CurrentForm->getValue("supervisor_id") : $CurrentForm->getValue("x_supervisor_id");
        if (!$this->supervisor_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->supervisor_id->Visible = false; // Disable update for API request
            } else {
                $this->supervisor_id->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->supervisor_id->CurrentValue = $this->supervisor_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // supervisor_id
        $this->supervisor_id->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->supervisor_id->Visible && $this->supervisor_id->Required) {
                if (!$this->supervisor_id->IsDetailKey && EmptyValue($this->supervisor_id->FormValue)) {
                    $this->supervisor_id->addErrorMessage(str_replace("%s", $this->supervisor_id->caption(), $this->supervisor_id->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->supervisor_id->FormValue)) {
                $this->supervisor_id->addErrorMessage($this->supervisor_id->getErrorMessage(false));
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // supervisor_id
        $this->supervisor_id->setDbValueDef($rsnew, $this->supervisor_id->CurrentValue, $this->supervisor_id->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['supervisor_id'])) { // supervisor_id
            $this->supervisor_id->CurrentValue = $row['supervisor_id'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("MaintenanceTeamsList"), "", $this->TableVar, true);
        $pageId = "edit";
