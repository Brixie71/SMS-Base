...
/**
 * Table class for maintenance_teams
 */
class MaintenanceTeams extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "maintenance_teams";
        ...
        $this->team_id->IsAutoIncrement = true; // Autoincrement field
        ...
        $this->team_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        // supervisor_id
        $this->supervisor_id = new DbField(
            $this, // Table
            'x_supervisor_id', // Variable name
            'supervisor_id', // Name
            '"supervisor_id"', // Expression
            'CAST("supervisor_id" AS varchar(255))', // Basic search expression
            3, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"supervisor_id"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->supervisor_id->InputTextType = "text";
        $this->supervisor_id->Raw = true;
        $this->supervisor_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->supervisor_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['supervisor_id'] = &$this->supervisor_id;
        ...
        $this->is_active->Lookup = new Lookup($this->is_active, 'maintenance_teams', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "maintenance_teams";
    }
    ...
    public function insert(&$rs)
    {
        ...
        try {
            ...
            $result = $conn->executeQuery(
                $queryBuilder->getSQL() . " RETURNING team_id",
        ...
        if ($result) {
            $this->team_id->setDbValue($result);
            $rs['team_id'] = $this->team_id->DbValue;
        }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['team_id']) && !EmptyValue($this->team_id->CurrentValue)) {
                $rs['team_id'] = $this->team_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->supervisor_id->DbValue = $row['supervisor_id'];
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            ...
            $val = !EmptyValue($this->team_id->OldValue) && !$current ? $this->team_id->OldValue : $this->team_id->CurrentValue;
        }
        if (!is_numeric($val)) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("MaintenanceTeamsList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "MaintenanceTeamsView" => $Language->phrase("View"),
            "MaintenanceTeamsEdit" => $Language->phrase("Edit"),
            "MaintenanceTeamsAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "MaintenanceTeamsView",
            Config("API_ADD_ACTION") => "MaintenanceTeamsAdd",
            Config("API_EDIT_ACTION") => "MaintenanceTeamsEdit",
            Config("API_DELETE_ACTION") => "MaintenanceTeamsDelete",
            Config("API_LIST_ACTION") => "MaintenanceTeamsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("MaintenanceTeamsView", $parm);
        } else {
            $url = $this->keyUrl("MaintenanceTeamsView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "MaintenanceTeamsAdd?" . $parm;
        } else {
            $url = "MaintenanceTeamsAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("MaintenanceTeamsEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("MaintenanceTeamsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("MaintenanceTeamsAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("MaintenanceTeamsList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("MaintenanceTeamsDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"team_id\":" . VarToJson($this->team_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        if ($this->team_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->team_id->CurrentValue);
        } else {
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->team_id->CurrentValue = $key;
            } else {
                $this->team_id->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        // supervisor_id
        $this->supervisor_id->setupEditAttributes();
        $this->supervisor_id->EditValue = $this->supervisor_id->CurrentValue;
        $this->supervisor_id->PlaceHolder = RemoveHtml($this->supervisor_id->caption());
        if (strval($this->supervisor_id->EditValue) != "" && is_numeric($this->supervisor_id->EditValue)) {
            $this->supervisor_id->EditValue = FormatNumber($this->supervisor_id->EditValue, null);
        }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->supervisor_id);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->supervisor_id);
