...
?>
<main class="edit">
<form name="fmaintenance_schedulesedit" id="fmaintenance_schedulesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { maintenance_schedules: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fmaintenance_schedulesedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fmaintenance_schedulesedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["schedule_id", [fields.schedule_id.visible && fields.schedule_id.required ? ew.Validators.required(fields.schedule_id.caption) : null], fields.schedule_id.isInvalid],
...
<?php if ($Page->schedule_id->Visible) { // schedule_id ?>
    <div id="r_schedule_id"<?= $Page->schedule_id->rowAttributes() ?>>
        <label id="elh_maintenance_schedules_schedule_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->schedule_id->caption() ?><?= $Page->schedule_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->schedule_id->getDisplayValue($Page->schedule_id->EditValue))) ?>"></span>
<input type="hidden" data-table="maintenance_schedules" data-field="x_schedule_id" data-hidden="1" name="x_schedule_id" id="x_schedule_id" value="<?= HtmlEncode($Page->schedule_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fmaintenance_schedulesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fmaintenance_schedulesedit", "x_scheduled_date", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fmaintenance_schedulesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fmaintenance_schedulesedit", "x_created_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fmaintenance_schedulesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fmaintenance_schedulesedit", "x_updated_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fmaintenance_schedulesedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fmaintenance_schedulesedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
