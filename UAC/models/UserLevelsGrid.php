 ...
 * Page class
 */
class UserLevelsGrid extends UserLevels
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "UserLevelsGrid";
    ...
    public $FormName = "f_user_levelsgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "UserLevelsGrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->user_level_id->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x_system_id") &&
            $CurrentForm->hasValue("o_system_id") &&
            $this->system_id->CurrentValue != $this->system_id->DefaultValue &&
            !($this->system_id->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->system_id->CurrentValue == $this->system_id->getSessionValue())
        ) {
            return false;
        }
        if (
            ...
            !($this->name->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->name->CurrentValue == $this->name->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_system_id")) {
            $this->system_id->setOldValue($CurrentForm->getValue("o_system_id"));
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->system_id->getSessionValue() != "") {
                ...
                $this->system_id->OldValue = $this->system_id->CurrentValue;
                $curVal = strval($this->system_id->CurrentValue);
                if ($curVal != "") {
                    $this->system_id->ViewValue = $this->system_id->lookupCacheOption($curVal);
                    if ($this->system_id->ViewValue === null) { // Lookup from database
