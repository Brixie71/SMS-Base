 ...
 * Page class
 */
class UserLevelAssignmentsGrid extends UserLevelAssignments
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "UserLevelAssignmentsGrid";
    ...
    public $FormName = "fuser_level_assignmentsgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "UserLevelAssignmentsGrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->assignment_id->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x_user_id") &&
            $CurrentForm->hasValue("o_user_id") &&
            $this->user_id->CurrentValue != $this->user_id->DefaultValue &&
            !($this->user_id->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->user_id->CurrentValue == $this->user_id->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->assignment_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
        ...
        if ($CurrentForm->hasValue("o_assigned_by")) {
            $this->assigned_by->setOldValue($CurrentForm->getValue("o_assigned_by"));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "systems") {
            $this->system_id->Visible = true; // Need to insert foreign key
            $this->system_id->CurrentValue = $this->system_id->getSessionValue();
        }
        if ($this->getCurrentMasterTable() == "_user_levels") {
        ...
        if ($this->getCurrentMasterTable() == "users") {
            $this->user_id->Visible = true; // Need to insert foreign key
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
            $detailKeys = [];
            $detailKeys["user_id"] = $this->user_id->CurrentValue;
    ...
    protected function getAddRow()
    {
        ...
        // assigned_by
        $this->assigned_by->CurrentValue = $this->assigned_by->getAutoUpdateValue(); // PHP
        $this->assigned_by->setDbValueDef($rsnew, $this->assigned_by->CurrentValue, false);
        ...
        if ($this->system_id->getSessionValue() != "") {
            $rsnew['system_id'] = $this->system_id->getSessionValue();
        }
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['assigned_by'])) { // assigned_by
            $this->assigned_by->setFormValue($row['assigned_by']);
        }
        if (isset($row['created_at'])) { // created_at
