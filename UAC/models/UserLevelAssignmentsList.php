 ...
 * Page class
 */
class UserLevelAssignmentsList extends UserLevelAssignments
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "UserLevelAssignmentsList";
    ...
    public $FormName = "fuser_level_assignmentslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "UserLevelAssignmentsList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "UserLevelAssignmentsAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "UserLevelAssignmentsDelete";
        $this->MultiUpdateUrl = "UserLevelAssignmentsUpdate";
    ...
    protected function hideFieldsForAddEdit()
    {
        ...
        if ($this->isAddOrEdit()) {
            $this->assigned_by->Visible = false;
        }
        if ($this->isAddOrEdit()) {
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fuser_level_assignmentsgrid";
        }
        ...
        if ($Security->currentUserID() != "" && !$Security->isAdmin()) { // Non system admin
            if ($this->getCurrentMasterTable() == "systems") {
                $this->DbMasterFilter = $this->addMasterUserIDFilter($this->DbMasterFilter, "systems"); // Add master User ID filter
            }
            if ($this->getCurrentMasterTable() == "_user_levels") {
                $this->DbMasterFilter = $this->addMasterUserIDFilter($this->DbMasterFilter, "_user_levels"); // Add master User ID filter
            }
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "systems") {
            $masterTbl = Container("systems");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("SystemsList"); // Return to master page
                return;
            } else {
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->assignment_id); // assignment_id
            $this->updateSort($this->user_level_id); // user_level_id
            $this->updateSort($this->user_id); // user_id
            $this->updateSort($this->assigned_by); // assigned_by
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"user_level_assignments\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"user_level_assignments\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fuser_level_assignmentslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fuser_level_assignmentslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->assignment_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"user_level_assignments\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "assignment_id");
            $this->createColumnOption($option, "user_level_id");
            $this->createColumnOption($option, "user_id");
            $this->createColumnOption($option, "assigned_by");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fuser_level_assignmentssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fuser_level_assignmentssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fuser_level_assignmentslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_user_level_assignments", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_user_level_assignments",
    ...
    {
        //Log("Page Load");
        if (Get("showmaster") === "users" && Get("fk_user_id") !== null) {
            $userId = Get("fk_user_id");
            // Perform any necessary validation on $userId here

            // Redirect to custom UAC page
            header("Location: /UAC/UserAccess/" . $userId);
            exit();
        } else {
            header("Location: /UAC/UserAccess");
            exit();
        }
    }
