...
class UsersEdit extends Users
{
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->password_hash->EditValue = HtmlEncode($this->password_hash->CurrentValue);
            ...
            if (!$Security->isAdmin() && $Security->isLoggedIn()) { // Non system admin
                if (SameString($this->user_id->CurrentValue, CurrentUserID())) {
                    ...
                    AddFilter($filterWrk, Container("users")->addParentUserIDFilter($this->user_id->CurrentValue));
    ...
    protected function validateForm()
    {
        ...
        if (in_array("aggregated_audit_logs", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailPage = Container("AggregatedAuditLogsGrid");
            if (in_array("aggregated_audit_logs", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
    ...
    protected function getEditRow($rsold)
    {
        ...
        if (!IsMaskedPassword($this->password_hash->CurrentValue)) {
            $this->password_hash->setDbValueDef($rsnew, $this->password_hash->CurrentValue, $this->password_hash->ReadOnly || Config("ENCRYPTED_PASSWORD") && $rsold['password_hash'] == $this->password_hash->CurrentValue);
        }
        ...
        // middle_name
        $this->middle_name->setDbValueDef($rsnew, $this->middle_name->CurrentValue, $this->middle_name->ReadOnly);
        ...
        // reports_to_user_id
        $this->reports_to_user_id->setDbValueDef($rsnew, $this->reports_to_user_id->CurrentValue, $this->reports_to_user_id->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['reports_to_user_id'])) { // reports_to_user_id
            $this->reports_to_user_id->CurrentValue = $row['reports_to_user_id'];
        }
