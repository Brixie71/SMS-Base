 ...
 * Page class
 */
class SystemsView extends Systems
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SystemsView";
    ...
    // CSS class/style
    public $CurrentPageName = "SystemsView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("system_id") ?? Route("system_id")) !== null) {
            $this->RecKey["system_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("system_id") ?? Route("system_id")) !== null) {
            ...
            $this->RecKey["system_id"] = $this->system_id->QueryStringValue;
        } elseif (Post("system_id") !== null) {
            $this->system_id->setFormValue(Post("system_id"));
            $this->RecKey["system_id"] = $this->system_id->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->system_id->setQueryStringValue($keyValue);
            $this->RecKey["system_id"] = $this->system_id->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "SystemsList"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "SystemsList"; // No matching record, return to list
                    }
                break;
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        // "detail__user_levels"
        $item = &$option->add("detail__user_levels");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("_user_levels", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("UserLevelsList?" . Config("TABLE_SHOW_MASTER") . "=systems&" . GetForeignKeyUrl("fk_system_id", $this->system_id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("UserLevelsGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'systems')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=_user_levels"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "_user_levels";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'systems')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=_user_levels"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("UserLevelAssignmentsList?" . Config("TABLE_SHOW_MASTER") . "=systems&" . GetForeignKeyUrl("fk_system_id", $this->system_id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
