...
<span id="el_users_reports_to_user_id">
    <select
        ...
        data-select2-id="fusersadd_x_reports_to_user_id"
        data-table="users"
...
<script>
loadjs.ready("fusersadd", function() {
    var options = { name: "x_reports_to_user_id", selectId: "fusersadd_x_reports_to_user_id" };
    if (fusersadd.lists.reports_to_user_id?.lookupOptions.length) {
        options.data = { id: "x_reports_to_user_id", form: "fusersadd" };
    } else {
        options.ajax = { id: "x_reports_to_user_id", form: "fusersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <?php if (strval($Page->user_level_id->getSessionValue()) != "") { ?>
    <input type="hidden" name="x_user_level_id" id="x_user_level_id" value="<?= HtmlEncode(strval($Page->user_level_id->getSessionValue())) ?>">
    <?php } ?>
<?php
...
<?php } ?>
<?php
    if (in_array("aggregated_audit_logs", explode(",", $Page->getCurrentDetailTable())) && $aggregated_audit_logs->DetailAdd) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
