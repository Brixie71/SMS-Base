...
<span id="el__user_levels_system_id">
    <select
        ...
        data-select2-id="f_user_levelsedit_x_system_id"
        <?php } ?>
...
<?php if (!$Page->system_id->IsNativeSelect) { ?>
<script>
loadjs.ready("f_user_levelsedit", function() {
    var options = { name: "x_system_id", selectId: "f_user_levelsedit_x_system_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (f_user_levelsedit.lists.system_id?.lookupOptions.length) {
        options.data = { id: "x_system_id", form: "f_user_levelsedit" };
    } else {
        options.ajax = { id: "x_system_id", form: "f_user_levelsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
