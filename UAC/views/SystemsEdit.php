...
?>
<main class="edit">
<form name="fsystemsedit" id="fsystemsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<?php if ($Page->system_id->Visible) { // system_id ?>
    <div id="r_system_id"<?= $Page->system_id->rowAttributes() ?>>
        <label id="elh_systems_system_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->system_id->caption() ?><?= $Page->system_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->system_id->getDisplayValue($Page->system_id->EditValue))) ?>"></span>
<input type="hidden" data-table="systems" data-field="x_system_id" data-hidden="1" name="x_system_id" id="x_system_id" value="<?= HtmlEncode($Page->system_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<?php
    if (in_array("_user_levels", explode(",", $Page->getCurrentDetailTable())) && $_user_levels->DetailEdit) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsystemsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsystemsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
