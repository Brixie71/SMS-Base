...
document.addEventListener('DOMContentLoaded', function() {
    ...
    ew.on("success", function(e) {
        ...
        if (data.action === "modal") {
            ...
            ew.toast(ew.language.phrase("RecordUpdated")); // Use built-in phrase
...
async function loadUsers() {
    try {
        const response = await fetch('/UAC/api/access/users', {
            headers: { 
...
async function loadUserAccess(userId) {
    ...
    try {
        const response = await fetch(`/UAC/api/access/matrix/${userId}`, {
            headers: { 
                'Content-Type': 'application/json',
                'X-Authorization': 'Bearer ' + ew.API_JWT_TOKEN 
            }
...
async function saveUserAccess() {
    ...
    try {
        const response = await fetch('/UAC/api/access', {
            method: 'POST',
            headers: { 
                'Content-Type': 'application/json',
                'X-Authorization': 'Bearer ' + ew.API_JWT_TOKEN 
