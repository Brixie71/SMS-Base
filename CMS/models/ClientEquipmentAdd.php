 ...
 * Page class
 */
class ClientEquipmentAdd extends ClientEquipment
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ClientEquipmentAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ClientEquipmentAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "ClientEquipmentView") {
    ...
    protected function getAddRow()
    {
        ...
        // tower_equipment_id
        $this->tower_equipment_id->setDbValueDef($rsnew, $this->tower_equipment_id->CurrentValue, false);
        ...
        // removal_date
        $this->removal_date->setDbValueDef($rsnew, UnFormatDateTime($this->removal_date->CurrentValue, $this->removal_date->formatPattern()), false);

        // status
        $this->status->setDbValueDef($rsnew, $this->status->CurrentValue, false);

        // maintenance_schedule
        $this->maintenance_schedule->setDbValueDef($rsnew, $this->maintenance_schedule->CurrentValue, false);

        // last_maintenance_date
        $this->last_maintenance_date->setDbValueDef($rsnew, UnFormatDateTime($this->last_maintenance_date->CurrentValue, $this->last_maintenance_date->formatPattern()), false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['tower_equipment_id'])) { // tower_equipment_id
            $this->tower_equipment_id->setFormValue($row['tower_equipment_id']);
        }
        ...
        if (isset($row['removal_date'])) { // removal_date
            $this->removal_date->setFormValue($row['removal_date']);
        }
        if (isset($row['status'])) { // status
            $this->status->setFormValue($row['status']);
        }
        if (isset($row['maintenance_schedule'])) { // maintenance_schedule
            $this->maintenance_schedule->setFormValue($row['maintenance_schedule']);
        }
        if (isset($row['last_maintenance_date'])) { // last_maintenance_date
            $this->last_maintenance_date->setFormValue($row['last_maintenance_date']);
        }
