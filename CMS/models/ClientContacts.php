...
/**
 * Table class for client_contacts
 */
class ClientContacts extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "client_contacts";
        ...
        $this->contact_id = new DbField(
            ...
            'CAST("contact_id" AS varchar(255))', // Basic search expression
            3, // Type
            0, // Size
            -1, // Date/Time format
        ...
        $this->contact_id->Raw = true;
        $this->contact_id->IsAutoIncrement = true; // Autoincrement field
        $this->contact_id->IsPrimaryKey = true; // Primary key field
        $this->contact_id->Nullable = false; // NOT NULL field
        $this->contact_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->contact_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->is_primary->Lookup = new Lookup($this->is_primary, 'client_contacts', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->is_primary->OptionCount = 2;
        $this->is_primary->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['is_primary'] = &$this->is_primary;

        // is_technical
        $this->is_technical = new DbField(
            $this, // Table
            'x_is_technical', // Variable name
            'is_technical', // Name
            '"is_technical"', // Expression
            'CAST("is_technical" AS varchar(255))', // Basic search expression
            11, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"is_technical"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->is_technical->InputTextType = "text";
        $this->is_technical->Raw = true;
        $this->is_technical->setDataType(DataType::BOOLEAN);
        $this->is_technical->Lookup = new Lookup($this->is_technical, 'client_contacts', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->is_technical->OptionCount = 2;
        $this->is_technical->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['is_technical'] = &$this->is_technical;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "client_contacts";
    }
    ...
    public function insert(&$rs)
    {
        ...
        try {
            ...
            $result = $conn->executeQuery(
                $queryBuilder->getSQL() . " RETURNING contact_id",
        ...
        if ($result) {
            $this->contact_id->setDbValue($result);
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['contact_id']) && !EmptyValue($this->contact_id->CurrentValue)) {
                $rs['contact_id'] = $this->contact_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('contact_id', $rs)) {
                AddFilter($where, QuotedName('contact_id', $this->Dbid) . '=' . QuotedValue($rs['contact_id'], $this->contact_id->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->is_technical->DbValue = (ConvertToBool($row['is_technical']) ? "1" : "0");
    ...
    {
        $keys = [];
        $val = $current ? $this->contact_id->CurrentValue : $this->contact_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->contact_id->CurrentValue = $keys[0];
            } else {
                $this->contact_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('contact_id', $row) ? $row['contact_id'] : null;
        } else {
            $val = !EmptyValue($this->contact_id->OldValue) && !$current ? $this->contact_id->OldValue : $this->contact_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@contact_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ClientContactsList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ClientContactsView" => $Language->phrase("View"),
            "ClientContactsEdit" => $Language->phrase("Edit"),
            "ClientContactsAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ClientContactsView",
            Config("API_ADD_ACTION") => "ClientContactsAdd",
            Config("API_EDIT_ACTION") => "ClientContactsEdit",
            Config("API_DELETE_ACTION") => "ClientContactsDelete",
            Config("API_LIST_ACTION") => "ClientContactsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ClientContactsView", $parm);
        } else {
            $url = $this->keyUrl("ClientContactsView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ClientContactsAdd?" . $parm;
        } else {
            $url = "ClientContactsAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ClientContactsEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ClientContactsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ClientContactsAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ClientContactsList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ClientContactsDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"contact_id\":" . VarToJson($this->contact_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->contact_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->contact_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("contact_id") ?? Route("contact_id")) !== null) {
                $arKeys[] = $keyValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->is_technical);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->is_technical);
