...
/**
 * Table class for contract_services
 */
class ContractServices extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "contract_services";
        ...
        $this->contract_id->Required = true; // Required field
        ...
        $this->quantity = new DbField(
            ...
            'quantity', // Name
            '"quantity"', // Expression
            'CAST("quantity" AS varchar(255))', // Basic search expression
            14, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
        ...
        // sla_terms
        $this->sla_terms = new DbField(
            $this, // Table
            'x_sla_terms', // Variable name
            'sla_terms', // Name
            '"sla_terms"', // Expression
            '"sla_terms"', // Basic search expression
            201, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"sla_terms"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->sla_terms->InputTextType = "text";
        $this->sla_terms->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['sla_terms'] = &$this->sla_terms;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "contract_services";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->sla_terms->DbValue = $row['sla_terms'];
    ...
    {
        return "\"contract_id\" = @contract_id@ AND \"service_id\" = @service_id@";
    }
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 2) {
            ...
            if ($current) {
                $this->service_id->CurrentValue = $keys[1];
            } else {
                $this->service_id->OldValue = $keys[1];
            }
        }
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ContractServicesList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ContractServicesView" => $Language->phrase("View"),
            "ContractServicesEdit" => $Language->phrase("Edit"),
            "ContractServicesAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ContractServicesView",
            Config("API_ADD_ACTION") => "ContractServicesAdd",
            Config("API_EDIT_ACTION") => "ContractServicesEdit",
            Config("API_DELETE_ACTION") => "ContractServicesDelete",
            Config("API_LIST_ACTION") => "ContractServicesList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ContractServicesView", $parm);
        } else {
            $url = $this->keyUrl("ContractServicesView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ContractServicesAdd?" . $parm;
        } else {
            $url = "ContractServicesAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContractServicesEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContractServicesList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContractServicesAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContractServicesList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ContractServicesDelete", $parm);
        }
    }
    ...
    public function keyUrl($url, $parm = "")
    {
        ...
        if ($this->service_id->CurrentValue !== null) {
            $url .= $this->RouteCompositeKeySeparator . $this->encodeKeyValue($this->service_id->CurrentValue);
        } else {
            return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
        }
        if ($parm != "") {
    ...
    public function getRecordKeys()
    {
        ...
        if (is_array($arKeys)) {
            foreach ($arKeys as $key) {
                ...
                if (!is_numeric($key[0])) { // contract_id
                    continue;
                }
                if (!is_numeric($key[1])) { // service_id
                    continue;
                }
                $ar[] = $key;
            }
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->contract_id->CurrentValue = $key[0];
            } else {
                $this->contract_id->OldValue = $key[0];
            }
            if ($setCurrent) {
                $this->service_id->CurrentValue = $key[1];
            } else {
                $this->service_id->OldValue = $key[1];
            }
    ...
    public function renderEditRow()
    {
        ...
        $this->sla_terms->EditValue = $this->sla_terms->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->sla_terms);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->sla_terms);
