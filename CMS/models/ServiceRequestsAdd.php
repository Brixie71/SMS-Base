 ...
 * Page class
 */
class ServiceRequestsAdd extends ServiceRequests
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ServiceRequestsAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ServiceRequestsAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "ServiceRequestsView") {
    ...
    protected function getAddRow()
    {
        ...
        // requested_by
        $this->requested_by->setDbValueDef($rsnew, $this->requested_by->CurrentValue, false);

        // requested_date
        $this->requested_date->setDbValueDef($rsnew, UnFormatDateTime($this->requested_date->CurrentValue, $this->requested_date->formatPattern()), false);

        // scheduled_date
        $this->scheduled_date->setDbValueDef($rsnew, UnFormatDateTime($this->scheduled_date->CurrentValue, $this->scheduled_date->formatPattern()), false);

        // completion_date
        $this->completion_date->setDbValueDef($rsnew, UnFormatDateTime($this->completion_date->CurrentValue, $this->completion_date->formatPattern()), false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['request_type'])) { // request_type
            $this->request_type->setFormValue($row['request_type']);
        }
        ...
        if (isset($row['requested_by'])) { // requested_by
            $this->requested_by->setFormValue($row['requested_by']);
        }
        if (isset($row['requested_date'])) { // requested_date
            $this->requested_date->setFormValue($row['requested_date']);
        }
        if (isset($row['scheduled_date'])) { // scheduled_date
            $this->scheduled_date->setFormValue($row['scheduled_date']);
        }
        if (isset($row['completion_date'])) { // completion_date
            $this->completion_date->setFormValue($row['completion_date']);
        }
