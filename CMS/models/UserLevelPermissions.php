...
class UserLevelPermissions extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->TableVar = "user_level_permissions";
        $this->TableName = 'user_level_permissions';
        $this->TableType = "TABLE";
        $this->ImportUseTransaction = $this->supportsTransaction() && Config("IMPORT_USE_TRANSACTION");
        $this->UseTransaction = $this->supportsTransaction() && Config("USE_TRANSACTION");

        // Update Table
        $this->UpdateTable = "user_level_permissions";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "user_level_permissions";
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            ...
            $val = !EmptyValue($this->table_name->OldValue) && !$current ? $this->table_name->OldValue : $this->table_name->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
    ...
    public function keyToJson($htmlEncode = false)
    {
        ...
        $json .= ",\"table_name\":" . VarToJson($this->table_name->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'user_level_permissions');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'user_level_permissions', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'user_level_permissions', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'user_level_permissions', $fldname, $key, $oldvalue);
            }
        }
    }
