...
class Clients extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->client_id->IsAutoIncrement = true; // Autoincrement field
        $this->client_id->IsPrimaryKey = true; // Primary key field
        $this->client_id->Nullable = false; // NOT NULL field
        $this->client_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->client_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['client_id'] = &$this->client_id;

        // client_code
        $this->client_code = new DbField(
            $this, // Table
            'x_client_code', // Variable name
            'client_code', // Name
            '"client_code"', // Expression
            '"client_code"', // Basic search expression
            200, // Type
            20, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"client_code"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->client_code->InputTextType = "text";
        $this->client_code->Nullable = false; // NOT NULL field
        $this->client_code->Required = true; // Required field
        $this->client_code->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['client_code'] = &$this->client_code;
        ...
        $this->company_name->Nullable = false; // NOT NULL field
        $this->company_name->Required = true; // Required field
        $this->company_name->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        ...
        // account_manager_id
        $this->account_manager_id = new DbField(
            $this, // Table
            'x_account_manager_id', // Variable name
            'account_manager_id', // Name
            '"account_manager_id"', // Expression
            'CAST("account_manager_id" AS varchar(255))', // Basic search expression
            3, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"account_manager_id"', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->account_manager_id->InputTextType = "text";
        $this->account_manager_id->Raw = true;
        $this->account_manager_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->account_manager_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['account_manager_id'] = &$this->account_manager_id;
        ...
        $this->registration_date = new DbField(
            ...
            CastDateFieldForLike("\"registration_date\"", 0, "philtower_cms"), // Basic search expression
            133, // Type
            0, // Size
            0, // Date/Time format
            false, // Is upload field
        ...
        $this->registration_date->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "clients";
    }
    ...
    public function insert(&$rs)
    {
        ...
        try {
            ...
            $result = $conn->executeQuery(
                $queryBuilder->getSQL() . " RETURNING client_id",
        ...
        if ($result) {
            $this->client_id->setDbValue($result);
            $rs['client_id'] = $this->client_id->DbValue;
        }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['client_id']) && !EmptyValue($this->client_id->CurrentValue)) {
                $rs['client_id'] = $this->client_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('client_id', $rs)) {
                AddFilter($where, QuotedName('client_id', $this->Dbid) . '=' . QuotedValue($rs['client_id'], $this->client_id->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->client_code->DbValue = $row['client_code'];
        $this->company_name->DbValue = $row['company_name'];
        $this->type_id->DbValue = $row['type_id'];
        $this->status->DbValue = $row['status'];
        $this->account_manager_id->DbValue = $row['account_manager_id'];
    ...
    {
        $keys = [];
        $val = $current ? $this->client_id->CurrentValue : $this->client_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->client_id->CurrentValue = $keys[0];
            } else {
                $this->client_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('client_id', $row) ? $row['client_id'] : null;
        } else {
            $val = !EmptyValue($this->client_id->OldValue) && !$current ? $this->client_id->OldValue : $this->client_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@client_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ClientsList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ClientsView" => $Language->phrase("View"),
            "ClientsEdit" => $Language->phrase("Edit"),
            "ClientsAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ClientsView",
            Config("API_ADD_ACTION") => "ClientsAdd",
            Config("API_EDIT_ACTION") => "ClientsEdit",
            Config("API_DELETE_ACTION") => "ClientsDelete",
            Config("API_LIST_ACTION") => "ClientsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ClientsView", $parm);
        } else {
            $url = $this->keyUrl("ClientsView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ClientsEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ClientsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ClientsAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ClientsList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ClientsDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"client_id\":" . VarToJson($this->client_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->client_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->client_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("client_id") ?? Route("client_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function renderEditRow()
    {
        ...
        $this->client_code->EditValue = $this->client_code->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->client_code);
                    $doc->exportCaption($this->company_name);
                    $doc->exportCaption($this->type_id);
                    $doc->exportCaption($this->status);
                    $doc->exportCaption($this->account_manager_id);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->client_code);
                        $doc->exportField($this->company_name);
                        $doc->exportField($this->type_id);
                        $doc->exportField($this->status);
                        $doc->exportField($this->account_manager_id);
