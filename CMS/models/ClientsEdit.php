 ...
 * Page class
 */
class ClientsEdit extends Clients
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ClientsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ClientsEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("client_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->client_id->setQueryStringValue($keyValue);
                $this->client_id->setOldValue($this->client_id->QueryStringValue);
            } elseif (Post("client_id") !== null) {
                $this->client_id->setFormValue(Post("client_id"));
                $this->client_id->setOldValue($this->client_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("client_id") ?? Route("client_id")) !== null) {
                    ...
                    $this->client_id->CurrentValue = null;
                }
            }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->client_id->IsDetailKey) {
            $this->client_id->setFormValue($val);
        }
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (client_code)
        if ($this->client_code->CurrentValue != "") {
            $filterChk = "(\"client_code\" = '" . AdjustSql($this->client_code->CurrentValue, $this->Dbid) . "')";
    ...
    protected function getEditRow($rsold)
    {
        ...
        // client_code
        $this->client_code->setDbValueDef($rsnew, $this->client_code->CurrentValue, $this->client_code->ReadOnly);
        ...
        // account_manager_id
        $this->account_manager_id->setDbValueDef($rsnew, $this->account_manager_id->CurrentValue, $this->account_manager_id->ReadOnly);

        // registration_date
        $this->registration_date->setDbValueDef($rsnew, UnFormatDateTime($this->registration_date->CurrentValue, $this->registration_date->formatPattern()), $this->registration_date->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        if (isset($row['client_code'])) { // client_code
            $this->client_code->CurrentValue = $row['client_code'];
        }
        ...
        if (isset($row['account_manager_id'])) { // account_manager_id
            $this->account_manager_id->CurrentValue = $row['account_manager_id'];
        }
        if (isset($row['registration_date'])) { // registration_date
            $this->registration_date->CurrentValue = $row['registration_date'];
        }
